...
//   上/下 双击   熄屏(只关背光,播放与联网继续;任意键唤醒,熄屏后第一下只唤醒)
...
typedef enum {
    ...
    RADIO_KEY_UP_LONG,
    RADIO_KEY_DOWN_LONG,
    // 上/下 双击 = 熄屏(只关背光,播放与联网继续)。这两个值不是给状态机用的:
    // 熄不熄屏与"当前在哪一页"无关,由 main.c 在分发的最前面处理,状态机的各页
    // 处理器对它们一律回 RADIO_ACT_NONE。放进同一个枚举,是为了让它们和别的按键
    // 走同一条分发路径(先判断"熄屏后第一下只唤醒",再持 LVGL 锁)。
    RADIO_KEY_UP_DOUBLE,
    RADIO_KEY_DOWN_DOUBLE,
} radio_key_t;
